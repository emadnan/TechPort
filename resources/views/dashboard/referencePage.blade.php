            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="float-right">
                        <button type="button" class="btn btn-success btn-sm"  id="add-btn" >Add Reference</button>
                    </div>
                </div>
                ...
                <div class="card-body">
                  <div class="table-responsive">
                    <table id="data-table" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($data as $id=>$reference )
                            <tr id="row_{{$reference-> id}}">
                                <td class="py-1" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px; ">{{$reference -> reference}}</td>
                                <td class="py-1" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px; ">{{$reference -> note}}</td>
                                <td class="py-1" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 90px;">
                                    <i id="view-btn" class="fa-solid fa-eye fa-lg" style="color:#28A745;  " data-id="{{$reference-> id}}"> <span style="color:black;">|</span> </i>
                                    <i id="update-btn" class="fa-solid fa-pen-to-square fa-lg" style="color:#E0A800;" data-id="{{$reference-> id}}"> <span style="color:black;">|</span> </i> 
                                    <i id="delete-btn" class="fa-regular fa-trash-can fa-lg" style="color:#C82333;"  data-id="{{$reference-> id}}"></i>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
<script type="text/javascript">
     $(document).ready(function(){
        ...
        // Check if the element with ID "menu_techreferred" exists.
            $('#menu_reference').addClass('bg-primary');
            $('#menu_reference').parent().parent().children('a:first-child').addClass('bg-primary');
            $('#menu_reference').parent().parent().addClass('menu-is-opening menu-open');
    });
        $.ajaxSetup({
                headers: { 
    ...
    $('#add-btn').click(function(){
        ...
        $('#form').trigger('reset');
    $('#modal_title').html('Add Reference');
    $("#referenceError").text('');
    $("#noteError").text('');
...
$('body').on('click' , '#update-btn' , function(){
    $('#form').trigger('reset');
    $('#form-modal').modal('show');
    $('#modal_title').html('Update Reference');
    $("#referenceError").text('');
    $("#noteError").text('');
    var rowID = $(this).data('id');
    var url = 'referenceUpdatePage/'+rowID;
            $.ajax({
                type: 'GET',
                url: url,
            ...
            if(inpID)
            {
                $.ajax({
                    ...
                    success: function(response)
                    {
                        ...
                        if(response.errors)
                        {
                            $("#referenceError").text(response.errors.reference);
                            $("#noteError").text(response.errors.note);

                            $("#reference").val(response.oldInput.reference);
