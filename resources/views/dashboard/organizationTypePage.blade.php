...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        // Check if the element with ID "menu_techreferred" exists.
            ...
            $('#menu_orgtypes').parent().parent().children('a:first-child').addClass('bg-primary');
            $('#menu_orgtypes').parent().parent().addClass('menu-is-opening menu-open');
    });
        $.ajaxSetup({
                headers: { 
...
$('body').on('click' , '#update-btn' , function(){
    ...
    var url = 'organizationTypeUpdatePage/'+rowID;
            $.ajax({
                type: 'GET',
                url: url,
...
$('#form').submit(function(){
            ...
            if(inpID)
            {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("organizationTypeUpdate") }}',
                    data: formData,
            ...
            else
            {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("organizationTypeCreate") }}',
                    data: formData,
...
$('body').on('click' , '#view-btn' , function(){
            ...
            var data = $('#details');
            data.empty(); 
            var url = 'organizationTypeRead/' + id;
             $.ajax({
                type: 'GET',
                url: url,
                ...
                success: function(response) {
                   ...
                   console.log(item.type);
                        var descp ='<p>'+ 
                '<span style="font-weight:700;">Type : </span>' +item.type+ '</p>'+
...
$('body').on('click' , '#delete-btn' , function(){
    var id = $(this).data('id');
    var url = 'organizationTypeDelete/' + id;
    $.ajax({
                type: 'GET',
                url: url,
